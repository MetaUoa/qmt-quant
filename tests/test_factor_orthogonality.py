...
def test_panel_rank_correlation_detects_duplicate_cross_sections():
    ...
    right = left * 10.0
    corr = panel_rank_correlation(left, right, min_symbols=3, end="2020-01-03")
    assert (corr.round(12) == 1.0).all()


def test_ic_correlation_and_greedy_selection_drop_redundant_factor_without_future_leakage():
    rows = []
    for date, a, b, c in [
    ...
    rows.extend(
        [
            {"factor": "a", "date": "2025-01-01", "horizon": 5, "rank_ic": 1.0},
            {"factor": "b", "date": "2025-01-01", "horizon": 5, "rank_ic": -1.0},
            {"factor": "c", "date": "2025-01-01", "horizon": 5, "rank_ic": 0.0},
        ]
    )
    observations = pd.DataFrame(rows)
    corr = ic_correlation_matrix(observations, horizon=5, end="2020-12-31")
