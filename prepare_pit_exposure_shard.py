...
def main() -> int:
    ...
    with baostock_session() as bs:
        ...
        basic = active_in_range(fetch_stock_basic(bs), args.start, args.end)
        shard = select_stock_shard(basic, args.shard_index, args.shard_count)
        codes = list(shard["ts_code"])
        shard[["ts_code", "list_date", "delist_date"]].to_parquet(root / "stock_basic_shard.parquet", index=False)
        for number, code in enumerate(codes, 1):
            ...
            if number == 1 or number % 50 == 0 or number == len(codes):
                print(f"[PIT exposure] {number}/{len(codes)} written={written} errors={len(errors)}")
    ...
    (root / "pit_exposure_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
